class AvrBinutils < Formula
  ...
  on_linux do
    # Homebrew's build PATH excludes HOMEBREW_PREFIX/bin, so a system patch(1)
    # is normally required. Minimal images such as Fedora Silverblue and its
    # derivatives do not ship one.
    depends_on "gpatch" => :build
    depends_on "zlib-ng-compat"
  end

  # Support for -C in avr-size, from the Arch Linux avr-binutils package.
  patch :DATA
  ...
  test do
    ...
    assert_match "AVR Memory Usage", shell_output("#{bin}/avr-size -C test-s.o")
  end
end
